...
def signed_distance_sphere(x, y, z, r, x_0, y_0, z_0):
    ...
    # ###############
    square_sum=(x-x_0)**2+(y-y_0)**2+(z-z_0)**2
    return np.sqrt(square_sum)-r
...
def signed_distance_torus(x, y, z, R, r, x_0, y_0, z_0):
    ...
    #raise NotImplementedError
    # ###############
    square1=(x-x_0)**2+(z-z_0)**2
    a=np.sqrt(square1)-R
    square2=(y-y_0)**2+a**2
    return np.sqrt(square2)-r
...
def signed_distance_atom(x, y, z):
    ...
    dist_orbit=signed_distance_torus(x,y,z,orbit_radius,orbit_thickness,proton_center[0],proton_center[1],proton_center[2])
    
    if isinstance(x,np.ndarray):
        answer=[]
        for i,j,k in zip(dist_proton,dist_electron,dist_orbit):
            answer.append(min(i,j,k))
        answer=np.array(answer)
        return answer
    else:
        return min(dist_proton,dist_electron,dist_orbit)
