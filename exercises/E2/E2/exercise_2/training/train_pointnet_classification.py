...
def train(model, trainloader, valloader, device, config):
    ...
    for epoch in range(config['max_epochs']):
        for i,batch in enumerate(trainloader):
            ...
            prediction=model(point_clouds)
            loss=None
            loss=loss_criterion(prediction,labels).to(device)
            loss.backward()
            optimizer.step()
            ...
            if iteration%config['validate_every_n']==(config['validate_every_n']-1):
                ...
                for batch_val in valloader:
                    ...
                    correct+=(prediction_labels==labels).sum().item()
                    loss+=loss_criterion(prediction,labels).item()

                accuracy=100*correct/total
                print(f'[{epoch:03d}/{i:05d}] val_loss: {loss/len(valloader):.3f}, val_accuracy: {accuracy:.3f}%')
