...
class ShapeNetParts(torch.utils.data.Dataset):
    ...
    def get_point_cloud_with_labels(shapenet_id):
        ...
        p_file = "exercise_2/data/shapenetcore_partanno_segmentation_benchmark_v0/{}/points/{}.pts".format(category_id,shape_id)
        ...
        l_file = "exercise_2/data/shapenetcore_partanno_segmentation_benchmark_v0/{}/points_label/{}.seg".format(category_id,shape_id)

        with open(l_file,'r') as pl:
            label = pl.readlines()
            label = [int(i) for i in label]
            label = np.array(label)
        label = label[idx]

        global_labels = [ShapeNetParts.part_id_to_overall_id[f"{category_id}_{classID}"] for classID in label.tolist()]
        global_labels = np.array(global_labels,dtype=np.int64)
