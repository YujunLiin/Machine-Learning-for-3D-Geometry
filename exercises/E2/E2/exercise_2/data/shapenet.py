...
class ShapeNetPoints(torch.utils.data.Dataset):
    ...
    def get_point_cloud(shapenet_id):
        ...
        #     point_cloud = read_as_3d_array(fptr).astype(np.float32)
        # point_cloud=np.reshape(point_cloud,(3,1024))
        # return point_cloud

        # load_obj_as_np = lambda path: np.array(list(map(lambda x: list(map(float, x.split(' ')[1:4])), path.read_text().splitlines())))
        # path=f"{ShapeNetPoints.dataset_path}/{category_id}/{shape_id}.obj"
        # point_cloud=load_obj_as_np(path).astype(np.float32)
        # point_cloud=np.reshape(point_cloud,(3,1024))
        # return point_cloud

        # point_cloud = trimesh.load(f"{ShapeNetPoints.dataset_path}/{category_id}/{shape_id}.obj")
