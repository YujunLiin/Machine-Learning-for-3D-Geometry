...
def train(model, latent_vectors, train_dataloader, device, config):
    ...
    for epoch in range(config['max_epochs']):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            latent_code=torch.cat((batch_latent_vectors,points),dim=1)
            predicted_sdf =model(latent_code)
            # TODO: truncate predicted sdf between -0.1 and 0.1
            predicted_sdf =torch.clip(predicted_sdf,-0.1,0.1)

            # compute loss
            loss = loss_criterion(predicted_sdf, sdf)  # ??????????? truncate ground truth sdf or not
